...
extends ModuleListener<AutoFeetPlace, Render3DEvent> {
    ...
    public void call(Render3DEvent event) {
        if (BlocksManager.get().debugSurround().booleanValue()) {
            for (BlockPos pos : ((AutoFeetPlace)this.module).getPlacements()) {
