...
extends ModuleListener<Chams, ModelRenderEvent.Pre> {
    ...
    public void call(ModelRenderEvent.Pre event) {
        if (((Chams)this.module).playerChams.getValue().booleanValue() && !((Chams)this.module).normal.getValue().booleanValue()) {
            if (event.getEntity() == ListenerPreModel.mc.player && !((Chams)this.module).self.getValue().booleanValue()) {
                return;
            }
            GL11.glPushMatrix();
